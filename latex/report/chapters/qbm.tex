...
In order to get a better understanding of how the QBM works, we studied a small 12-qubit problem that can be solved exactly.
...
To this end, we randomly generated the values of \( h_i \) and \( J_{ij} \) from a normal distribution with \( \mu = 0 \) and \( \sigma = 0.1 \), then used the KL divergence to compare samples generated by the annealer with theoretical distributions.
...
In the right heatmap, where \( \squench = 0.55 \), we observed a narrow band in which the annealer sampled close to a quantum Boltzmann distribution, and in fact approximated multiple distributions, depending on the effective temperature.
...
Therefore, for the remainder of this thesis we focus on training models with \( s^* = 1 \) using classical Boltzmann distributed samples generated from the annealer, also enabling us to use the aforementioned method of learning the effective temperature.

\subsubsection{Choosing an Embedding}
We compared 10 different heuristically generated embeddings based on how well they approximate the desired distribution.
This embedding comparison used only direct embeddings (no chains), so the embeddings only differ by the location of the qubits on the chip, and pause-and-quench anneal schedules with \( \trelative = 20 \ \si{\micro\second} \) and \( \Deltapause = 0 \ \si{\micro\second} \).
...
We use the results of this model as a baseline to compare the models trained using the annealer in the next subsection with.
...
After gaining a deeper insight into the workings of the BQRBM from the 12-qubit problem, we moved to the final stage of training a quantum market generator with 64 visible and 30 hidden units.
...
To this end, we trained a number of models with various settings of these hyperparameters for 20 epochs to get a read on the trend direction, and then chose their values empirically.
...
The QQ plots in~\cref{fig:qbm_log_returns_qq} point out that the BQRBM model struggles the most with the USDJPY and USDCAD marginals.
\begin{figure}[!htb]
    \begin{center}
...
Around the time this project was started, Pochart et al. released a paper~\cite{pochart_2021} in which they discuss challenges associated with using a D-Wave annealer to sample Boltzmann random variables.
